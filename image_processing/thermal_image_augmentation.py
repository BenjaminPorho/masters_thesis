...
def augment_image(image):
    ...
    if augmentation_type == 'flip':
        ...
        angle = random.uniform(-45, 45)  # Rotate within -45 to 45 degrees
        h, w = image.shape[:2]
        center = (w // 2, h // 2)
    ...
    for day_folder in sorted(os.listdir(preprocessed_folder)):
        ...
        for mouse_folder in sorted(os.listdir(day_path)):
            ...
            for wound_type in ['treated', 'control']:
                ...
                # Copy original images to the output folder and apply slight rotation
                for img in images:
                    ...
                    # Apply a small random rotation to the original images
                    rotated_image = augment_image(image)
                    cv2.imwrite(dst, rotated_image)

                # Generate augmented images
                max_iterations = 50  # Prevent infinite loops
                iteration = 0
                while image_count < target and iteration < max_iterations:
                    for img in images:
                        if image_count >= target:
                            break
                        ...
                        success = cv2.imwrite(aug_path, augmented_image)
                        if success:
                            print(f"Saved augmented image: {aug_path}")
                            image_count += 1
                        else:
                            print(f"Failed to save augmented image: {aug_path}")

                        iteration += 1

                if iteration >= max_iterations:
                    print(f"Warning: Reached max iterations for {day_folder} -> {mouse_folder} -> {wound_type}")

if __name__ == "__main__":
    preprocessed_folder = "/Users/benporho/thesis_data/Preprocessed Thermal Images"
    output_folder = "/Users/benporho/thesis_data/Augmented Thermal Images"

    augment_to_target(preprocessed_folder, output_folder, target=10)
